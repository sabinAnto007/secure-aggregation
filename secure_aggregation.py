...
# ---------- Datasets (HAR filename updated to 'harmergedd.csv') ----------
DATASETS = [
...
# Scenario fractions and default K
SCENARIO_FRACS = {"small": 0.10, "moderate": 0.50, "full": 1.00}
SCENARIO_DEFAULT_K = {"small": 10, "moderate": 50, "full": None}

# Defaults
DEFAULT_REPEATS = 5
MASKING_DTYPE_BYTES = 4       # float32 for comms accounting
AHE_FE_KEY_BITS = 2048        # simulated ciphertext size (bytes = key_bits/8)
AGG_EPS = 1e-8                # tolerance for correctness check

# ---------- Utilities ----------
def load_numeric_df(path: str, dropna=True) -> pd.DataFrame:
    """Load CSV and keep only numeric columns (harmergedd & faostattt_clean are already numeric)."""
...
def pair_seed(ns: str, i: int, j: int) -> int:
    """Deterministic shared randomness per pair (i,j) within namespace ns."""
...
def apply_pairwise_masks(update_vecs, namespace: str):
    """Classic pairwise canceling masks: O(K^2 * d)."""
    K = len(update_vecs)
...
# ---------- AHE / FE (SIMULATED timing + comms) ----------
def _simulate_encrypt_vector(vec: np.ndarray, key_bits: int, times=1):
    # simple CPU work to get a nonzero timing
...
def run_fe_round(df: pd.DataFrame, K: int, ns: str, key_bits: int = 2048):
    ...
    sums = client_local_sums(X, idxs)
    t_enc = sum(_simulate_encrypt_vector(s, key_bits, times=2) for s in sums)  # heavier than AHE
    agg_sum = np.sum(sums, axis=0)
    t_dec = _simulate_decrypt_vector(agg_sum, key_bits, times=1)
...
def main():
    ...
    for algo in algos:
        for df_all, name in loaded:
            # Base K from scenario
            K = (SCENARIO_DEFAULT_K[scenario] or len(df_all))
            if scenario == "full":
                print(f"[INFO] {name}: FULL → using 100% rows, K={K}")
            else:
                print(f"[INFO] {name}: {scenario.upper()} → using {int(frac*100)}% rows, K={K}")
            ...
            for r in range(repeats):
                sub = subset(df_all, frac, 42 + r)
                ns = f"{name}:{scenario}:{r}"
                if algo == "masking":
                    res = run_masking_round(sub, K, ns, MASKING_DTYPE_BYTES)
                elif algo == "ahe":
                    res = run_ahe_round(sub, K, ns, AHE_FE_KEY_BITS)
                else:
                    res = run_fe_round(sub, K, ns, AHE_FE_KEY_BITS)
    ...
    # Sort by algorithm then dataset
    ...
    out_csv = f"phase2_{csv_algo}_{scenario}_K{results[0]['clients(K)']}_rep{repeats}.csv"
